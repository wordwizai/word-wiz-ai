...
async def load_and_preprocess_audio_file(audio_file: UploadFile, session_id: str | None = None) -> tuple[np.ndarray, str]:
    ...
    """
    import gc  # Add gc import for explicit memory management
    ...
    # Clean up audio_bytes after loading
    del audio_bytes
    gc.collect()
...
async def analyze_audio_file_event_stream(
    ...
    with MemoryTracker("Audio Analysis Pipeline", enable_memory_logging):
        try:
            ...
            with MemoryTracker("Audio Processing", enable_memory_logging):
            ...
            with MemoryTracker("TTS Audio Generation", enable_memory_logging):
