...
async function prerenderRoutes() {
  ...
  // Detect if running on Vercel - skip prerendering (too resource-intensive for serverless)
  const isVercel = process.env.VERCEL === "1";

  if (isVercel) {
    console.log(
      "⏭️  Skipping prerendering on Vercel (serverless environment limitation)"
    );
    console.log(
      "💡 The app uses client-side rendering with react-helmet-async for SEO"
    );
    console.log("✅ Build complete without prerendering\n");
    return;
  }
